...
<div class='table-card' id='watchlist-section'><h2>🟣 Ecological Watch-List</h2><div class='watch-note'>Separate verification layer for fire, moisture stress, vegetation stress, historical disturbance, or influence-zone signals.</div>{watchlist_table}</div><div class='footer'>This app reads selected CSV columns for summaries/tables and draws the whole clipped GeoJSON layer on the live map. If GeoJSON is unavailable, it shows centroid markers only, not fake whole-grid polygons.</div></div></div>
<script>
...
function truth(v){{return v===true||String(v).toUpperCase()==='TRUE'||String(v).toUpperCase()==='YES'||String(v)==='1'}}
function gridColor(p){{var pr=String(p.final_priority||p.risk_class||p.priority||'').toUpperCase();var w=String(p.ecological_watch_level||'').toUpperCase();var activeFire=truth(p.active_fire_signal);var fireRisk=truth(p.fire_risk_signal);if(activeFire)return'#ff0000';if(pr.includes('CRITICAL'))return'#d60000';if(fireRisk)return'#ff9900';if(pr.includes('HIGH'))return'#ffb000';if(pr.includes('MEDIUM')||pr.includes('MODERATE'))return'#ffd400';if(w.includes('HIGH'))return'#b000ff';if(w.includes('MEDIUM'))return'#a855f7';return'#00ff00';}}
function num(v){{var n=Number(v);return isNaN(n)?'N/A':n.toFixed(3)}}
function popupHtml(p){{var link=p.google_maps_link||'#';var active=truth(p.active_fire_signal)?'YES - active/recent thermal signal':'No active thermal signal';var risk=truth(p.fire_risk_signal)?'YES - fire-risk/check condition':'No special fire-risk flag';return `<div style='font-family:Arial;min-width:280px'><b>${{p.grid_id||'Grid'}}</b><br><br><b>Priority:</b> ${{p.final_priority||p.risk_class||'N/A'}}<br><b>Risk Score:</b> ${{p.final_risk_score||'N/A'}}<br><b>NDVI:</b> ${{num(p.ndvi)}}<br><b>NDMI:</b> ${{num(p.ndmi)}}<br><b>Active Fire:</b> ${{active}}<br><b>Fire Risk/Check:</b> ${{risk}}<br><b>Fire Note:</b> ${{p.fire_display||'No active thermal signal'}}<br><b>Action:</b> ${{p.officer_action||p.patrol_action||p.ecology_recommendation||'Routine patrol'}}<hr><b>Watch:</b> ${{p.ecological_watch_category||'Stable Forest Zone'}}<br><b>Why listed:</b> ${{p.why_go||p.ecological_watch_reason||'No major watch condition'}}<br><b>Carbon:</b> ${{p.carbon_change_status||'N/A'}} / ${{p.mrv_confidence||'N/A'}}<br><b>Soil:</b> ${{p.soil_moisture_retention_class||'N/A'}}<br><b>Tree Estimate:</b> ${{p.estimated_tree_count||'N/A'}}<br><b>Inference:</b> ${{p.ecology_inference||'N/A'}}<br><br><small>Active fire means current/recent FIRMS or thermal evidence. Fire-risk means screening priority only.</small><br><br><a href='${{link}}' target='_blank'>Open navigation</a></div>`}}
var legend=L.control({{position:'bottomright'}});legend.onAdd=function(map){{var div=L.DomUtil.create('div','map-legend');div.innerHTML="<b>FRIS Colour Legend</b>"+"<div class='legend-row'><span class='legend-swatch' style='background:#ff0000'></span>Active FIRMS / thermal fire</div>"+"<div class='legend-row'><span class='legend-swatch' style='background:#d60000'></span>Critical ecological risk</div>"+"<div class='legend-row'><span class='legend-swatch' style='background:#ff9900'></span>Fire-risk / fire-check only</div>"+"<div class='legend-row'><span class='legend-swatch' style='background:#ffb000'></span>High risk</div>"+"<div class='legend-row'><span class='legend-swatch' style='background:#ffd400'></span>Medium risk</div>"+"<div class='legend-row'><span class='legend-swatch' style='background:#b000ff'></span>High ecological watch</div>"+"<div class='legend-row'><span class='legend-swatch' style='background:#a855f7'></span>Medium ecological watch</div>"+"<div class='legend-row'><span class='legend-swatch' style='background:#00ff00'></span>Low / stable / routine</div>"+"<small>Orange does not mean the grid is burning.</small>";return div;}};legend.addTo(map);
fetch('/geojson').then(r=>r.json()).then(data=>{{if(!data.features||data.features.length===0){{alert('No grid features found. Check CSV latitude/longitude columns and fris_latest.geojson.');return}}var layer=L.geoJSON(data,{{pointToLayer:(f,latlng)=>{{var c=gridColor(f.properties||{{}});return L.circleMarker(latlng,{{radius:6,color:c,weight:2,fillColor:c,fillOpacity:.85}})}},style:f=>{{var c=gridColor(f.properties||{{}});return{{color:c,weight:1.4,fillColor:c,fillOpacity:.50}}}},onEachFeature:(f,l)=>l.bindPopup(popupHtml(f.properties||{{}}))}}).addTo(map);try{{map.fitBounds(layer.getBounds(),{{padding:[20,20]}})}}catch(e){{}}L.control.layers({{'Satellite':satellite,'CartoDB Light':carto}},{{'Full FRIS clipped GeoJSON layer':layer}},{{collapsed:false}}).addTo(map)}}).catch(e=>alert('GeoJSON could not load. Open /debug.'));
</script></body></html>"""
    del df
    gc.collect()
...
def view_full_map():
    """Open the complete saved FRIS HTML map only when the user asks.

    The dashboard page itself never embeds this heavy map. That prevents Render
    memory crashes. send_file streams the saved HTML instead of reading it into
    a Python string.
    """
    if not os.path.exists(MAP_FILE):
        return Response("FRIS map file not found inside data/fris_latest_map.html", status=404, mimetype="text/plain")
    response = send_file(MAP_FILE, mimetype="text/html", as_attachment=False, download_name="fris_latest_map.html", max_age=0)
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    return response
