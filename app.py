...
st_autorefresh(interval=30000, key="fris_auto_refresh")
...
def make_reason(row):
    ...
    for c in ["mining_pressure", "mine_pressure", "mining_influence", "mine_influence", "distance_to_mine_m"]:
        if c in row.index and pd.notna(row.get(c)):
            reasons.append("ecological pressure near anthropogenic/mining influence zone")
            break
...
def make_action(row):
    risk = normalize_risk(row.get("risk_class", row.get("final_priority", row.get("patrol_priority", ""))))
...
def build_dynamic_map(df, geojson_data, lat_col, lon_col, grid_col, risk_col):
    center_lat = df[lat_col].mean() if lat_col else 24.8
    center_lon = df[lon_col].mean() if lon_col else 87.2
    ...
    if geojson_data:
        def style_function(feature):
            ...
            risk_value = props.get(risk_col, props.get("risk_class", props.get("final_priority", "UNKNOWN")))
            return {
        ...
        def popup_function(feature):
            ...
            risk = props.get(risk_col, props.get("risk_class", props.get("final_priority", "Unknown")))
        ...
        folium.GeoJson(
            geojson_data,
            name="FRIS Grid Risk Layer",
            style_function=style_function,
            popup=popup_function
        ).add_to(m)

    # Point layer fallback / additional markers
...
st.markdown('<div class="main-title">🌳 Godda FRIS Dynamic Dashboard</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="sub-text">Forest health, moisture stress, fire alerts, ecological pressure, and preliminary carbon-readiness monitoring.</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    """
    <div class="safe-box">
    <b>Important:</b> Mining/industrial influence is shown only as ecological pressure context.
...
df["risk_normalized"] = df[risk_col].apply(normalize_risk) if risk_col else "UNKNOWN"
...
show_only_pressure = st.sidebar.checkbox("Show only ecological pressure grids", value=False)
...
if show_only_fire:
    ...
    if fire_cols:
        mask = False
        for c in fire_cols:
            mask = mask | filtered_df[c].astype(str).str.lower().isin(["true", "1", "yes", "y"])
        filtered_df = filtered_df[mask]

if show_only_pressure:
    pressure_cols = [
        c for c in filtered_df.columns
        if "mine" in c.lower() or "mining" in c.lower() or "pressure" in c.lower() or "disturbance" in c.lower()
    ]
    if pressure_cols:
        mask = filtered_df[pressure_cols].notna().any(axis=1)
        filtered_df = filtered_df[mask]
...
with tab1:
    ...
    if filtered_df.empty:
        ...
        live_map = build_dynamic_map(filtered_df, geojson_data, lat_col, lon_col, grid_col, risk_col)
...
with tab2:
    ...
    if risk_col:
        risk_chart = filtered_df["risk_normalized"].value_counts().reset_index()
...
st.divider()
st.caption("FRIS — Forest Resilience Information System | Dynamic dashboard refreshed every 30 seconds")