...
function gridColor(p){{var pr=String(p.final_priority||p.risk_class||p.priority||'').toUpperCase();var w=String(p.ecological_watch_level||'').toUpperCase();var fire=String(p.fire_display||p.fire_intensity_class||p.fire_count||'').toUpperCase();if(pr.includes('FIRE')||fire.includes('FIRE'))return'#ff4d00';if(pr.includes('CRITICAL'))return'#ff0000';if(pr.includes('HIGH'))return'#ff9900';if(pr.includes('MEDIUM')||pr.includes('MODERATE'))return'#ffd400';if(w.includes('HIGH'))return'#b000ff';if(w.includes('MEDIUM'))return'#a855f7';return'#00ff00';}}
