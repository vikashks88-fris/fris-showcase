...
def get_dashboard_data():
    ...
    grid_col = pick_col(df, ["grid_id", "Grid_ID", "GRID_ID", "id"])
    ...
    if risk_col:
        risk_series = df[risk_col].fillna("LOW").astype(str).str.upper()
    else:
    ...
    low = int(risk_series.str.contains("LOW|NORMAL|HEALTHY|ROUTINE", na=False).sum())
    ...
    if fire_col:
        ...
        if pd.api.types.is_numeric_dtype(fire_values):
            ...
            active_fire = int(
                ...
                .str.upper()
                .str.contains("YES|TRUE|ACTIVE|FIRE|1", na=False)
                .sum()
            )

    avg_ndvi = safe_number_mean(df, ndvi_col)
    avg_ndmi = safe_number_mean(df, ndmi_col)
    ...
    if active_fire > 0 or critical > 0:
        ...
        recommendation = "Monitor moisture stress and continue scheduled patrol."
    else:
    ...
    if len(temp) > 0:
        ...
        if reason_col:
            top_threat_reason = str(top_row[reason_col])
        elif risk_col:
            top_threat_reason = str(top_row[risk_col])
        else:
            top_threat_reason = "Priority grid selected by FRIS ranking."
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .system-status {
            font-size: 36px;
            font-weight: 950;
            color: {% if data.system_status == "ALERT" %}#f87171{% elif data.system_status == "WATCH" %}#facc15{% elif data.system_status == "MONITOR" %}#38bdf8{% else %}#4ade80{% endif %};
        }
...
<div class="layout">
    ...
    <aside class="sidebar">
        ...
        <div class="nav-item active" onclick="scrollToSection('overview')">Command Overview</div>
        <div class="nav-item" onclick="scrollToSection('risk')">Live Forest Risk</div>
        <div class="nav-item" onclick="scrollToSection('map')">Operational Map</div>
        <div class="nav-item" onclick="scrollToSection('grid')">Grid Intelligence</div>
        <div class="nav-item" onclick="location.href='/api/data'">API Data</div>
        <div class="nav-item" onclick="location.reload()">Refresh System</div>
    </aside>
    ...
    <main class="main">
        ...
        {% elif data.system_status == "MONITOR" %}
        <div class="alert-banner" style="background:linear-gradient(90deg,#38bdf8,#2563eb);">
            ℹ FOREST MONITORING ACTIVE — MOISTURE / HEALTH WATCH
        </div>
        {% endif %}
        ...
        <section class="weather-strip">
            <div class="weather-item">Temperature: Field input pending</div>
            <div class="weather-item">Rainfall: Field input pending</div>
            <div class="weather-item">Wind: Field input pending</div>
        ...
        <section class="kpi-grid" id="risk">
            <div class="card blue">
...
""", data=data, map_exists=map_exists)
...
def api_geojson():
    if os.path.exists(GEOJSON_FILE):
        try:
            ...
            return jsonify({"error": "GeoJSON reading failed"}), 500
