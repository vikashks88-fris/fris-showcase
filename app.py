...
# Keep the app lightweight during startup and avoid slow external calls unless enabled.
IS_RENDER = bool(os.environ.get("RENDER") or os.environ.get("PORT"))
ENABLE_LIVE_WEATHER = os.environ.get("ENABLE_LIVE_WEATHER", "0").strip().lower() in ("1", "true", "yes", "on")
MAX_GEOJSON_FEATURES = int(os.environ.get("MAX_GEOJSON_FEATURES", "1200"))

# Lightweight in-memory cache. Render reloads the process when redeployed,
# and this cache refreshes automatically when CSV files change on disk.
_CSV_CACHE = {"mtime": None, "df": None}
_CARBON_CACHE = {"path": None, "mtime": None, "df": None}
_PLANTATION_CACHE = {"path": None, "mtime": None, "df": None}
...
def read_csv():
    """Read main FRIS CSV with simple mtime cache for faster dashboard reloads."""
    ...
    try:
        ...
        if _CSV_CACHE["df"] is not None and _CSV_CACHE["mtime"] == mtime:
        ...
        return _CSV_CACHE.get("df")


def read_carbon_csv():
    """Read separate FRIS Carbon output with mtime cache."""
    path = carbon_file_path_used()

    if not os.path.exists(path):
        _CARBON_CACHE.update({"path": None, "mtime": None, "df": None})
        return None

    try:
        mtime = os.path.getmtime(path)
        if (
            _CARBON_CACHE["df"] is not None
            and _CARBON_CACHE["path"] == path
            and _CARBON_CACHE["mtime"] == mtime
        ):
            return _CARBON_CACHE["df"]

        df = pd.read_csv(path, low_memory=False)
        df = df.loc[:, ~df.columns.duplicated()].copy()
        _CARBON_CACHE.update({"path": path, "mtime": mtime, "df": df})
        return df
    except Exception:
        return _CARBON_CACHE.get("df")
...
def read_plantation_csv():
    """Read separate FRIS Plantation Engine output with mtime cache."""
    path = plantation_file_path_used()

    if not os.path.exists(path):
        _PLANTATION_CACHE.update({"path": None, "mtime": None, "df": None})
        return None

    try:
        mtime = os.path.getmtime(path)
        if (
            _PLANTATION_CACHE["df"] is not None
            and _PLANTATION_CACHE["path"] == path
            and _PLANTATION_CACHE["mtime"] == mtime
        ):
            return _PLANTATION_CACHE["df"]

        df = pd.read_csv(path, low_memory=False)
        df = df.loc[:, ~df.columns.duplicated()].copy()
        _PLANTATION_CACHE.update({"path": path, "mtime": mtime, "df": df})
        return df
    except Exception:
        return _PLANTATION_CACHE.get("df")
...
def build_plantation_engine_table(plant_df, limit=15):
    ...
    for _, row in work.iterrows():
        ...
        mining = safe_text(row.get("field_context_class"), "N/A")
        maps = safe_text(row.get("google_maps_link"), "#")
        maps_link = maps if maps.startswith("http") else "#"
...
# =============================
# GRID DECISION SUMMARY HELPERS
# =============================

def get_row_by_grid_id(df, grid_id):
    if df is None or df.empty or not grid_id or "grid_id" not in df.columns:
        return None
    m = df[df["grid_id"].astype(str) == str(grid_id)]
    if m.empty:
        return None
    return m.iloc[0]


def operational_condition_from_row(row):
    if row is None:
        return "N/A", "No operational FRIS row found."
    ...
    risk = safe_float(row.get("final_risk_score"), None)

    if fire_count > 0 or "FIRE" in priority:
        return "FIRE / SAME-DAY VERIFICATION", "Active/recent fire signal or fire-check priority is present. Verify immediately."
    if "CRITICAL" in priority:
        return "CRITICAL PATROL CONDITION", f"Critical priority with health {health} and moisture {moisture}. Same-day field check recommended."
    if "HIGH" in priority:
        return "HIGH PATROL CONDITION", f"High priority grid. Health: {health}; moisture: {moisture}; risk score: {risk if risk is not None else 'N/A'}."
    if "MEDIUM" in priority or "MODERATE" in priority:
        return "MONITORING / FIELD CHECK", f"Moderate operational concern. Health: {health}; moisture: {moisture}."
    return "ROUTINE OPERATIONAL MONITORING", f"No urgent patrol signal. Health: {health}; moisture: {moisture}."


def ecological_condition_from_row(row):
    if row is None:
        return "N/A", "No ecological FRIS row found."

    forest_pct = safe_float(row.get("forest_pct"), 0) or 0
    memory = safe_text(row.get("ecological_memory_class"), "N/A")
    health = safe_text(row.get("health_class"), "N/A").upper()
    moisture = safe_text(row.get("moisture_class_calibrated", row.get("moisture_class", "N/A"))).upper()
    canopy = safe_text(row.get("canopy_change_class"), "N/A")

    watch = classify_ecological_watch(row)

    if forest_pct >= 85 and watch["category"] != "Stable Forest Zone":
        return "DENSE FOREST ECOLOGICAL WATCH", f"85%+ forest logic active: {watch['reason']} Memory: {memory}. Canopy: {canopy}."
    if forest_pct >= 85 and ("STRESSED" in health or "CRITICAL" in health or "DRY" in moisture):
        return "DENSE FOREST UNDER STRESS", f"Dense forest is present, but health/moisture signal is weak. Memory: {memory}. Canopy: {canopy}."
    if memory in ["YEARLY_DEGRADATION_SIGNAL", "REPEATED_YEARLY_STRESS", "CHRONIC_RISK_GRID"]:
        return "HISTORICAL ECOLOGICAL MEMORY SIGNAL", f"Timestamped FRIS memory shows {memory}. This needs trend-based monitoring."
    return "ROUTINE ECOLOGICAL CONDITION", f"No major dense-forest anomaly from the 85% inference layer. Memory: {memory}."


def plantation_condition_from_row(row):
    if row is None:
        return "N/A", "No plantation engine row found."

    label = safe_text(row.get("plantation_suitability"), "N/A")
    score = safe_float(row.get("plantation_score"), None)
    reason = safe_text(row.get("plantation_reason"), "No reason available.")

    if "HIGH_PLANTATION_PRIORITY" in label:
        title = "HIGH PLANTATION PRIORITY"
    elif "RESTORATION_PLANTATION_GRID" in label:
        title = "RESTORATION PLANTATION"
    elif "ASSISTED_REGENERATION_GRID" in label:
        title = "ASSISTED NATURAL REGENERATION"
    elif "PROTECT_EXISTING_FOREST" in label:
        title = "PROTECT EXISTING FOREST"
    elif "FIRE_RECOVERY_CHECK" in label:
        title = "FIRE RECOVERY CHECK BEFORE PLANTATION"
    else:
        title = label

    score_text = "N/A" if score is None else f"{score:.0f}/100"
    return title, f"Plantation score: {score_text}. {reason}"


def carbon_condition_from_row(row):
    if row is None:
        return "N/A", "No carbon engine row found."

    label = safe_text(row.get("genuine_carbon_class", row.get("carbon_opportunity_class", row.get("carbon_zone", "N/A"))))
    score = safe_float(row.get("genuine_carbon_score", row.get("carbon_opportunity_score", None)), None)
    rec = safe_text(row.get("carbon_recommendation"), make_genuine_carbon_recommendation(label))

    score_text = "N/A" if score is None else f"{score:.1f}/100"
    return label, f"Carbon score: {score_text}. {rec}"


def final_grid_decision(op_title, eco_title, plant_title, carbon_title):
    text = " ".join([op_title, eco_title, plant_title, carbon_title]).upper()

    if "FIRE" in text:
        return "Verify fire / burn condition first, then decide restoration."
    if "HIGH PLANTATION" in text:
        return "Priority candidate for plantation or restoration support after field verification."
    if "RESTORATION PLANTATION" in text:
        return "Good restoration plantation candidate after ground checking."
    if "ASSISTED NATURAL REGENERATION" in text:
        return "Prefer assisted natural regeneration, protection, gap filling, and monitoring."
    if "PROTECT EXISTING FOREST" in text or "CARBON_STABLE_RESERVE" in text:
        return "Protect existing forest and monitor ecological memory; avoid unnecessary plantation."
    if "CARBON_LOSS_ALERT" in text or "CARBON_RISK_ZONE" in text:
        return "Carbon/ecological risk monitoring required before any claim."
    return "Routine monitoring with next FRIS update."


def build_grid_decision_summary(df, plantation_df, carbon_df, limit=20):
    sources = []

    if df is not None and not df.empty:
        core = df.copy()
        core["_decision_source_score"] = core.apply(priority_rank, axis=1)
        sources.append(core[["grid_id", "_decision_source_score"]].copy())

    if plantation_df is not None and not plantation_df.empty and "grid_id" in plantation_df.columns:
        p = plantation_df.copy()
        p["_decision_source_score"] = p.apply(plantation_engine_rank, axis=1)
        sources.append(p[["grid_id", "_decision_source_score"]].copy())

    if carbon_df is not None and not carbon_df.empty and "grid_id" in carbon_df.columns:
        c = carbon_df.copy()
        c["_decision_source_score"] = c.apply(carbon_rank_value, axis=1)
        sources.append(c[["grid_id", "_decision_source_score"]].copy())

    if not sources:
        return "<div class='empty'>No decision-summary data available. Check FRIS, plantation, and carbon CSV files.</div>"

    combined = pd.concat(sources, ignore_index=True)
    combined = combined.dropna(subset=["grid_id"])
    combined = combined.groupby("grid_id", as_index=False)["_decision_source_score"].max()
    combined = combined.sort_values("_decision_source_score", ascending=False).head(limit)

    rows = ""
    for _, item in combined.iterrows():
        grid_id = safe_text(item.get("grid_id"), "N/A")
        core_row = get_row_by_grid_id(df, grid_id)
        plant_row = get_row_by_grid_id(plantation_df, grid_id)
        carbon_row = get_row_by_grid_id(carbon_df, grid_id)

        op_title, op_reason = operational_condition_from_row(core_row)
        eco_title, eco_reason = ecological_condition_from_row(core_row)
        plant_title, plant_reason = plantation_condition_from_row(plant_row)
        carbon_title, carbon_reason_text = carbon_condition_from_row(carbon_row)
        decision = final_grid_decision(op_title, eco_title, plant_title, carbon_title)

        maps = "#"
        for candidate_row in [core_row, plant_row, carbon_row]:
            if candidate_row is not None:
                m = safe_text(candidate_row.get("google_maps_link"), "#")
        ...
        <tr>
            <td><b>{html.escape(grid_id)}</b></td>
            <td><b>{html.escape(op_title)}</b><br>{html.escape(op_reason)}</td>
            <td><b>{html.escape(eco_title)}</b><br>{html.escape(eco_reason)}</td>
            <td><b>{html.escape(plant_title)}</b><br>{html.escape(plant_reason)}</td>
            <td><b>{html.escape(carbon_title)}</b><br>{html.escape(carbon_reason_text)}</td>
            <td>{html.escape(decision)}</td>
            <td><a href=\"{html.escape(maps)}\" target=\"_blank\">Open</a></td>
        </tr>
        """
    ...
    <div class=\"table-wrap\">
        <table>
            <thead>
                <tr>
                    <th>Grid</th>
                    <th>Operational Patrol Condition</th>
                    <th>Dense-Forest Ecological Condition</th>
                    <th>Plantation / Restoration Condition</th>
                    <th>Carbon Condition</th>
...
def get_weather(df):
    ...
    # Render 502/slow-load protection:
    # Do not block the dashboard on an external weather API unless explicitly enabled.
    if not ENABLE_LIVE_WEATHER:
        return {
            "source": "Weather disabled for fast Render loading",
            "status": "Disabled",
            "temperature": "N/A",
            "rainfall": "N/A",
            "rainfall_24h": "N/A",
...
def is_mine_or_non_grid_feature(feature):
    """Return True for mine/mining polygons or non-FRIS overlay polygons.

    The public dashboard should show FRIS grid intelligence only.
    It removes mine lease/polygon overlays from uploaded GeoJSON/Folium fallback data,
    while keeping grid polygons that contain grid_id or FRIS indicators.
    """
    props = feature.get("properties", {}) if isinstance(feature, dict) else {}
    text_blob = " ".join(str(v) for v in props.values()).upper()
    keys_blob = " ".join(str(k) for k in props.keys()).upper()

    # Keep genuine FRIS grid features.
    if props.get("grid_id") or props.get("GRID_ID"):
        return False

    # Remove explicit mining / lease / quarry overlays.
    mine_words = [
        "MINE", "MINING", "COAL", "ECL", "LEASE", "QUARRY",
        "COLLIERY", "DUMP", "OB", "OVERBURDEN",
        "MINE_POLYGON", "MINING_POLYGON", "LEASE_BOUNDARY",
        "MAP1", "MAP2", "MINE AREA", "MINING AREA"
    ]
    if any(word in text_blob or word in keys_blob for word in mine_words):
        return True

    # If a polygon has no FRIS operational properties, treat it as overlay and hide it.
    fris_keys = [
        "final_priority", "risk_class", "forest_pct", "ndvi", "ndmi",
        "health_class", "moisture_class", "patrol_action", "google_maps_link"
    ]
    if not any(k in props for k in fris_keys):
        geom_type = str(feature.get("geometry", {}).get("type", "")).upper()
...
def remove_mine_polygons_from_geojson(data):
    """Clean GeoJSON before exposing it on the public dashboard."""
    if not isinstance(data, dict):
        return data
    features = data.get("features")
    if not isinstance(features, list):
        return data
    data["features"] = [f for f in features if not is_mine_or_non_grid_feature(f)]
    return data
...
def dashboard():
    ...
    grid_decision_summary_table = build_grid_decision_summary(df, plantation_df, carbon_df, limit=20)
...
<div class="layout">
    ...
    <div class="sidebar">
        ...
        <div class="nav"><a href="#decision-summary-section">✅ Grid Decision Summary</a></div>
    ...
    <div class="main">
        ...
        <div class="table-card" id="decision-summary-section">
            <h2>✅ Grid Decision Summary — What Is Happening & What Should Be Done?</h2>
            <div class="watch-note">
                This section separates four different conditions for each grid: operational patrol condition, 85% dense-forest ecological inference, plantation/restoration suitability, and carbon condition. It helps officers and CSR teams decide whether the grid needs patrol, protection, plantation, assisted regeneration, or carbon monitoring.
            </div>
            {grid_decision_summary_table}
        </div>
        ...
        <div class="table-card" id="watchlist-section">
            ...
            <div class="watch-note">
                ...
                or field-verification indicators. It does not prove illegal activity or confirmed deforestation.
            </div>
...
def latest_map():
    # Public dashboard map is rebuilt from cleaned /geojson.
    # This prevents old Folium files from showing mine lease/polygon overlays.
    response = Response(fallback_leaflet_map_html(), mimetype="text/html")
...
def geojson():
    ...
    if os.path.exists(GEOJSON_FILE):
        try:
            ...
            data = remove_mine_polygons_from_geojson(data)
            features = data.get("features", [])

            # Render protection: prevent very large GeoJSON responses from causing 502/timeout.
            if isinstance(features, list) and len(features) > MAX_GEOJSON_FEATURES:
                features = features[:MAX_GEOJSON_FEATURES]
                data["features"] = features
                data["render_note"] = f"GeoJSON limited to {MAX_GEOJSON_FEATURES} features for fast loading."
...
<body>
    ...
    <div class="box">
        ...
        <div class="note">
            ...
            or field-verification indicators. It does not prove illegal activity, confirmed deforestation, compensation liability,
...
@app.route("/healthz")
def healthz():
    """Very light Render health endpoint. Does not read big CSV/map files."""
    return jsonify({
        "status": "ok",
