...
.box {background:white; padding:35px; border-radius:20px; max-width:600px; margin:auto; box-shadow:0 4px 15px rgba(0,0,0,.12);}
h1 {color:#55765a;}
...
.card {background:white; padding:20px; margin:18px auto; max-width:800px; border-radius:14px; box-shadow:0 3px 10px rgba(0,0,0,.10);}
...
def priority():
    ...
    return render_template_string(LIST_HTML, title="Today Priority Grids", rows=rows)
...
def top10():
    ...
    return render_template_string(LIST_HTML, title="Top 10 Urgent Grids", rows=rows)
...
def detail(index):
    ...
    rows = prepare_rows(df)
    return render_template_string(DETAIL_HTML, item=rows[index])
...
def open_map():
    ...
    if map_file is None:
        ...
        <p>Upload one of these files to GitHub root folder:</p>
        <ul>
            <li>fris_master_map_latest.html</li>
