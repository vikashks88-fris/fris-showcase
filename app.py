...
def make_csv_grid_geojson(df):
    ...
    for _, row in df.iterrows():
        ...
        for plant_col in [
            "estimated_tree_count", "estimated_tree_density_class", "plantation_signal_score",
            "plantation_signal_class", "plantation_detection_confidence", "plantation_reason",
            "plantation_signal_note", "canopy_change_class", "tree_count_disclaimer"
        ]:
...
def classify_ecological_watch(row):
    # Prefer engine-generated watch columns when present. Dashboard should display the engine brain, not recalculate it differently.
...
def ecological_summary_text(df):
    ...
    return "Most Jharkhand forest grids remain stable in the current run. No major active fire emergency or ecological anomaly is detected."
...
def dashboard():
    ...
    carbon_change_text = format_carbon(sum_first_available_col(df, [
        "carbon_change_ton",
    ...
    plant_counts = plantation_counts(df)
    possible_plantation_count = plant_counts["possible_plantation"]
    vegetation_recovery_count = plant_counts["vegetation_recovery"]
    canopy_recovery_count = plant_counts["canopy_recovery"]
    canopy_decline_count = plant_counts["canopy_decline"]
    total_tree_estimate_text = format_number(sum_tree_count(df), 0, " trees")
    plantation_table = build_plantation_table(df, limit=15)
...
<html>
<head>
    <title>FRIS Jharkhand Dashboard</title>
...
<div class="layout">
    ...
    <div class="sidebar">
        ...
        <div class="side-card">
            <b>Jharkhand FRIS State Screening</b><br><br>
    ...
    <div class="main">
        ...
        <div class="content" id="map-section">
            ...
            <div class="right">
                ...
                <div class="card">
                    <h3>🌱 Plantation / Regeneration</h3>
                    <div class="row"><span>Possible Plantation / Regeneration</span><span class="value">{possible_plantation_count}</span></div>
                    <div class="row"><span>Vegetation Recovery Signals</span><span class="value">{vegetation_recovery_count}</span></div>
                    <div class="row"><span>Canopy Recovery Grids</span><span class="value">{canopy_recovery_count}</span></div>
                    <div class="row"><span>Canopy Decline Grids</span><span class="value">{canopy_decline_count}</span></div>
        ...
        <div class="table-card" id="plantation-section">
            ...
            <div class="watch-note">
                {html.escape(plantation_summary)} This is an approximate satellite-assisted estimate. It is not exact tree counting and not official plantation confirmation.
            </div>
            {plantation_table}
        </div>
...
<script>
    ...
    function gridColor(props) {{
        ...
        if (props.ecological_watch_level) {{
            watch = String(props.ecological_watch_level).toUpperCase();
        }}

        if (p.includes("CRITICAL")) return "#ff0000";
        if (p.includes("FIRE")) return "#ff4d00";
        if (p.includes("HIGH")) return "#ff9900";
        if (p.includes("MEDIUM") || p.includes("MODERATE")) return "#ffd400";

        if (watch.includes("HIGH")) return "#b000ff";
        if (watch.includes("MEDIUM")) return "#a855f7";
    ...
    function popupHtml(props) {{
        ...
        var action = props.patrol_action || props.ecology_recommendation || "Routine patrol";
        var link = props.google_maps_link || "#";
        var fireContext = props.fire_context || props.fire_context_class || "N/A";
        var confidence = props.mrv_confidence || props.confidence_class || props.carbon_opportunity_confidence_factor || "N/A";
        var effectiveArea = props.effective_forest_area_ha !== undefined && props.effective_forest_area_ha !== null ? Number(props.effective_forest_area_ha).toFixed(2) + " ha" : "N/A";

        var watchCat = props.ecological_watch_category || "Stable Forest Zone";
        var watchLevel = props.ecological_watch_level || "LOW";
        var watchReason = props.ecological_watch_reason || "No major ecological watch-list condition.";
        var watchAction = props.ecological_watch_action || "Routine monitoring.";
        var ecoInf = props.ecological_inference || props.ecology_inference || props.field_inference || "No separate ecology inference available.";
        var ecoRec = props.ecology_recommendation || props.patrol_action || "Routine monitoring.";

        var treeCount = props.estimated_tree_count !== undefined && props.estimated_tree_count !== null ? Number(props.estimated_tree_count).toLocaleString() : "N/A";
        var treeDensity = props.estimated_tree_density_class || "N/A";
        var plantSignal = props.plantation_signal_class || "N/A";
        var plantConfidence = props.plantation_detection_confidence || "N/A";
        var plantScore = props.plantation_signal_score !== undefined && props.plantation_signal_score !== null ? Number(props.plantation_signal_score).toFixed(1) : "N/A";
        var canopyChange = props.canopy_change_class || "N/A";
        var plantReason = props.plantation_reason || props.plantation_signal_note || "No plantation/regeneration note available.";
        var treeDisclaimer = props.tree_count_disclaimer || "Satellite-assisted estimate only; not exact tree counting.";
        ...
        return `
            <div style="font-family:Arial; min-width:260px;">
                ...
                <hr>
                <b>Ecological Watch:</b> ${{watchCat}}<br>
                <b>Watch Level:</b> ${{watchLevel}}<br>
                <b>Why:</b> ${{watchReason}}<br>
                <b>Watch Action:</b> ${{watchAction}}<br><br>
                <b>Ecology Inference:</b> ${{ecoInf}}<br>
                <b>Ecology Recommendation:</b> ${{ecoRec}}<br><br>

                <hr>
                <b>Estimated Tree Count:</b> ${{treeCount}}<br>
                <b>Tree Density:</b> ${{treeDensity}}<br>
                <b>Plantation Signal:</b> ${{plantSignal}}<br>
                <b>Signal Confidence:</b> ${{plantConfidence}}<br>
                <b>Signal Score:</b> ${{plantScore}}<br>
                <b>Canopy Change:</b> ${{canopyChange}}<br>
                <b>Reason:</b> ${{plantReason}}<br>
                <small>${{treeDisclaimer}}</small><br><br>

                <small>This is verification support only, not legal proof.</small><br><br>
...
def plantation():
    df = read_csv()
    plantation_table = build_plantation_table(df, limit=100)
    summary = plantation_summary_text(df)
    counts = plantation_counts(df)
    total_tree_estimate_text = format_number(sum_tree_count(df), 0, " trees")
...
<body>
    ...
    <div class="box">
        ...
        <div class="cards">
            <div class="card">Possible Plantation / Regeneration<b>{counts['possible_plantation']}</b></div>
            <div class="card">Vegetation Recovery<b>{counts['vegetation_recovery']}</b></div>
            <div class="card">Canopy Recovery<b>{counts['canopy_recovery']}</b></div>
            <div class="card">Estimated Trees<b>{total_tree_estimate_text}</b></div>
        </div>
        {plantation_table}
    </div>
</body>
