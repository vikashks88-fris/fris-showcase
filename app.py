...
def read_carbon_csv():
    """Read the separate FRIS Carbon Opportunity Engine output.
    Prefer v2 output; fallback to older carbon output if v2 is not present.
    """
    path = CARBON_FILE if os.path.exists(CARBON_FILE) else CARBON_FILE_OLD
...
def carbon_opportunity_counts(carbon_df):
    """Count classes from separate Carbon Opportunity Engine v2 output."""
    defaults = {
        "high_opportunity": 0,
        "stable_reserve": 0,
        "watch_zone": 0,
    ...
    class_col = first_existing_col(carbon_df, [
    ...
    if verify_col:
        ...
        # Fallback: verification/loss/risk zones should be checked on ground.
        defaults["verification_required"] = (
            defaults["risk_zone"] + defaults["verification_zone"] + defaults["loss_alert"]
        )
...
def carbon_summary_text(carbon_df):
    if carbon_df is None or carbon_df.empty:
        return "Separate Carbon Opportunity Engine output not found. Run carbon2.py and place fris_carbon_opportunity_v2.csv inside the data folder."

    counts = carbon_opportunity_counts(carbon_df)
    return (
        "FRIS Carbon Opportunity Engine is active. It separates stable carbon reserve, recovery opportunity, "
        "watch, risk, verification, and loss-alert grids for planning-level MRV support. This is not certified carbon-credit issuance. "
        f"Total carbon grids analysed: {counts['total_carbon_grids']}."
    )
...
def carbon_rank_value(row):
    ...
    if rank is not None:
        return -rank  # rank 1 should sort highest

    score = safe_float(row.get("carbon_opportunity_score"), 0) or 0
    cls = safe_text(row.get("carbon_opportunity_class", row.get("carbon_zone", "")), "").upper()
    bonus = 0
    if "HIGH_CARBON_OPPORTUNITY" in cls:
        bonus = 10000
...
def build_carbon_table(carbon_df, limit=15):
    if carbon_df is None or carbon_df.empty:
        return "<div class='empty'>Carbon Opportunity CSV not loaded. Run carbon2.py first.</div>"

    required_any = ["carbon_opportunity_score", "carbon_opportunity_class", "carbon_zone"]
    if not any(col in carbon_df.columns for col in required_any):
        return "<div class='empty'>Carbon Opportunity Engine columns not found. Run the updated carbon2.py engine first.</div>"
    ...
    for _, row in work.iterrows():
        ...
        score = safe_float(row.get("carbon_opportunity_score"), None)
        score_text = "N/A" if score is None else f"{score:.1f}"
        carbon_class = safe_text(row.get("carbon_opportunity_class", row.get("carbon_zone", "N/A")))
        rank = safe_float(row.get("carbon_priority_rank"), None)
        rank_text = "N/A" if rank is None else f"{rank:.0f}"
        recommendation = safe_text(row.get("carbon_recommendation", "N/A"))
        reason = safe_text(row.get("carbon_reason", "N/A"))
        verification = safe_text(row.get("field_verification_required", "N/A"))
        maps = safe_text(row.get("google_maps_link"), "#")
        maps_link = maps if maps.startswith("http") else "#"
...
def latest_map():
    ...
    if not os.path.exists(latest_map_file):
        return Response(
            "Latest FRIS map not found. Put fris_latest_map.html inside the data folder or C:\\cfris\\output.",
            status=404,
            mimetype="text/plain"
        )
...
def download_carbon():
    ...
    if not os.path.exists(path):
        return Response(
            "Carbon CSV not found. Run carbon2.py and place fris_carbon_opportunity_v2.csv inside the data folder.",
            status=404,
            mimetype="text/plain"
        )
