...
def make_why_go(row):
    ...
    if fire_detected in ["TRUE", "YES", "ACTIVE", "1"] or fire_count > 0:
        reasons.append("active fire/thermal signal needs immediate verification")
    ...
    if ndmi is not None:
        if ndmi < 0.05:
            reasons.append("low NDMI shows dry vegetation/moisture stress")
        elif ndmi < 0.15:
    ...
    if mining_risk >= 60:
        reasons.append("mining pressure/risk is high near this grid")
    elif mining_risk >= 35:
        reasons.append("mining pressure may require field observation")

    if carbon_change < 0:
        reasons.append("carbon change is negative, possible biomass/ecosystem decline")
...
def dashboard():
    ...
    active_fire = count_contains(df, ["fire_detected", "active_fire", "fire_status"], "TRUE") + count_contains(df, ["fire_detected", "active_fire", "fire_status"], "YES")
...
<head>
    ...
    <style>
        * {{
        ...
        .card h3, .map-card h2, .table-card h2 {{
            margin-top: 0;
        }}
...
<div class="layout">
    ...
    <div class="main">
        ...
        <div class="footer">
            Dashboard refreshes every 30 seconds. Time is forced to Asia/Kolkata IST. Weather uses CSV first; API only if CSV weather is missing.
        </div>
