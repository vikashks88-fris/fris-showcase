# FRIS DASHBOARD V34 - compact map popup + separate /grid/<grid_id> information panel
...
def _first_row_value(row, columns, default="N/A"):
    ...
    for col in columns:
        if col in row.index:
            ...
            if text_value and text_value.upper() not in {"N/A", "NONE", "NULL", "NAN"}:
                return value
    return default


def _detail_text(row, columns, default="N/A"):
    return html.escape(safe_text(_first_row_value(row, columns, default), default))


def _detail_number(row, columns, decimals=2, suffix=""):
    value = _first_row_value(row, columns, None)
    if value is None or value == "N/A":
        return "N/A"
    return html.escape(format_number(value, decimals, suffix))


def _detail_boolean_label(value):
    return "Yes" if is_truthy(value) else "No"


def _detail_row(label, value):
    return f"<div class='detail-row'><span>{html.escape(label)}</span><b>{value}</b></div>"


def _detail_section(title, rows_html, note=""):
    note_html = f"<div class='detail-note'>{html.escape(note)}</div>" if note else ""
    return f"<section class='detail-card'><h2>{html.escape(title)}</h2>{note_html}{rows_html}</section>"


def build_grid_information_page(row):
    """Build a full per-grid information panel using CSV values only.

    This keeps the map popup short while preserving complete scientific detail
    in a separate page for each grid.
    """
    grid_id = safe_text(row.get("grid_id"), "Selected Grid")
    priority = safe_text(_first_row_value(row, ["final_priority", "risk_class", "priority"], "LOW"), "LOW")
    risk_score = _detail_number(row, ["final_risk_score"], 1)
    ...
    field_required = _detail_boolean_label(row.get("field_verification_required"))
    active_fire = "Yes" if row_has_fire(row) else "No"
    fire_risk = "Yes" if row_has_fire_risk(row) else "No"

    overview_rows = "".join([
        _detail_row("Scientific Risk Priority", html.escape(priority)),
        _detail_row("Final Risk Score", risk_score),
        _detail_row("Operational Attention", _detail_text(row, ["operational_attention_label", "operational_attention_class"], "Routine monitoring")),
        _detail_row("Field Verification Required", html.escape(field_required)),
        _detail_row("Recommended Action", html.escape(action)),
        _detail_row("Why this grid is listed", html.escape(why_go)),
    ])

    vegetation_rows = "".join([
        _detail_row("Current Forest Extent", _detail_number(row, ["forest_pct"], 2, "%")),
        _detail_row("Forest Area Support", _detail_number(row, ["effective_forest_area_ha", "forest_area_ha", "area_ha"], 2, " ha")),
        _detail_row("NDVI", _detail_number(row, ["ndvi"], 3)),
        _detail_row("Vegetation Health", _detail_text(row, ["health_class"], "N/A")),
        _detail_row("NDMI", _detail_number(row, ["ndmi"], 3)),
        _detail_row("Moisture Class", _detail_text(row, ["moisture_class_calibrated", "moisture_class"], "N/A")),
        _detail_row("Rain-adjusted NDMI", _detail_number(row, ["rain_adjusted_ndmi_for_moisture"], 4)),
        _detail_row("Moisture Reference", _detail_text(row, ["moisture_reference_source"], "N/A")),
    ])

    soil_rows = "".join([
        _detail_row("Soil Context", _detail_text(row, ["soil_type"], "N/A")),
        _detail_row("Moisture Retention", _detail_text(row, ["soil_moisture_retention_class"], "N/A")),
        _detail_row("Drying Speed", _detail_text(row, ["soil_drying_speed"], "N/A")),
        _detail_row("Ecological Stability", _detail_text(row, ["soil_supported_ecological_stability"], "N/A")),
        _detail_row("Terrain", _detail_text(row, ["terrain_class"], "N/A")),
        _detail_row("Slope", _detail_number(row, ["slope_deg"], 2, "°")),
        _detail_row("Elevation", _detail_number(row, ["elevation_m"], 1, " m")),
    ])

    watch_rows = "".join([
        _detail_row("Watch Category", html.escape(watch.get("category", "N/A"))),
        _detail_row("Watch Level", html.escape(watch.get("level", "N/A"))),
        _detail_row("Watch Reason", html.escape(watch.get("reason", "N/A"))),
        _detail_row("Watch Action", html.escape(watch.get("action", "N/A"))),
        _detail_row("365-Day Memory Class", _detail_text(row, ["ecological_memory_class"], "N/A")),
        _detail_row("365-Day Memory Score", _detail_number(row, ["ecological_memory_score"], 1)),
        _detail_row("Ecology Status", html.escape(eco.get("status", "N/A"))),
        _detail_row("Ecology Inference", html.escape(eco.get("inference", "N/A"))),
    ])

    fire_weather_rows = "".join([
        _detail_row("Active Fire Signal", html.escape(active_fire)),
        _detail_row("Fire Risk / Check", html.escape(fire_risk)),
        _detail_row("Fire Display", html.escape(fire_display(row))),
        _detail_row("FRP Max", _detail_number(row, ["fire_frp_max", "firms_frp_max"], 2)),
        _detail_row("Fire Intensity", _detail_text(row, ["fire_intensity_class"], "N/A")),
        _detail_row("Temperature", _detail_number(row, ["temperature_c", "temperature", "temp_c"], 1, "°C")),
        _detail_row("Rainfall 24h", _detail_number(row, ["rainfall_24h_mm", "rainfall_current_mm", "rainfall", "rain_mm"], 1, " mm")),
        _detail_row("Wind", _detail_number(row, ["wind_speed_kmph", "wind_kmph", "wind_speed"], 1, " km/h")),
        _detail_row("Gust", _detail_number(row, ["wind_gust_kmph"], 1, " km/h")),
        _detail_row("Weather Fire Spread", _detail_text(row, ["weather_fire_spread_class"], "N/A")),
    ])

    canopy_rows = "".join([
        _detail_row("GEDI AGBD", _detail_number(row, ["gedi_agbd_ton_per_ha", "agb_ton_per_ha"], 2, " t/ha")),
        _detail_row("GEDI Biomass Confidence", _detail_text(row, ["gedi_biomass_confidence"], "N/A")),
        _detail_row("Carbon Method", _detail_text(row, ["carbon_estimation_method"], "N/A")),
        _detail_row("Canopy Change", _detail_text(row, ["canopy_change_class"], "N/A")),
        _detail_row("Estimated Tree Count", _detail_number(row, ["estimated_tree_count"], 0)),
        _detail_row("Tree Estimate Confidence", _detail_text(row, ["tree_estimation_confidence"], "N/A")),
    ])

    carbon_rows = "".join([
        _detail_row("Ecosystem Carbon", _detail_number(row, ["ecosystem_carbon_total_ton", "estimated_ecosystem_carbon_ton"], 1, " tons C")),
        _detail_row("Biomass Carbon", _detail_number(row, ["biomass_carbon_total_ton"], 1, " tons C")),
        _detail_row("Carbon Change", _detail_number(row, ["carbon_change_co2e_ton", "carbon_change_from_365d", "carbon_change_ton"], 1, " tons CO₂e")),
        _detail_row("Carbon Change Status", _detail_text(row, ["carbon_change_status"], "N/A")),
        _detail_row("Preliminary Opportunity", _detail_number(row, ["preliminary_carbon_opportunity_ton_co2e"], 1, " tons CO₂e")),
        _detail_row("MRV Confidence", _detail_text(row, ["mrv_confidence"], "N/A")),
        _detail_row("Carbon Claim Status", _detail_text(row, ["carbon_credit_claim_status"], "Not Certified")),
    ])

    nav_button = f"<a class='btn' href='{html.escape(maps_link)}' target='_blank'>Open Google Maps</a>" if maps_link != "#" else ""

    page = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <title>FRIS Grid Information - {html.escape(grid_id)}</title>
        <meta name='viewport' content='width=device-width, initial-scale=1.0'>
        <style>
            * {{ box-sizing: border-box; font-family: Arial, sans-serif; }}
            body {{ margin: 0; background: #061307; color: white; padding: 22px; }}
            .wrap {{ max-width: 1180px; margin: auto; }}
            .top {{ background: linear-gradient(135deg, rgba(223,255,0,.18), rgba(255,255,255,.08)); border: 1px solid rgba(223,255,0,.35); border-radius: 24px; padding: 22px; margin-bottom: 18px; }}
            h1 {{ margin: 0 0 8px; color: #dfff00; font-size: 30px; }}
            h2 {{ margin: 0 0 12px; color: #dfff00; font-size: 19px; }}
            .sub {{ color: #eaffc4; line-height: 1.55; }}
            .btn {{ display: inline-block; background: #dfff00; color: #102000; padding: 11px 14px; border-radius: 14px; text-decoration: none; font-weight: 800; margin: 8px 8px 0 0; }}
            .grid {{ display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 16px; }}
            .detail-card {{ background: rgba(255,255,255,.10); border: 1px solid rgba(255,255,255,.18); border-radius: 22px; padding: 18px; }}
            .detail-row {{ display: grid; grid-template-columns: 220px 1fr; gap: 12px; padding: 9px 0; border-bottom: 1px solid rgba(255,255,255,.12); line-height: 1.45; }}
            .detail-row span {{ color: #cfe8cf; }}
            .detail-row b {{ color: #ffffff; font-weight: 700; word-break: break-word; }}
            .detail-note {{ background: rgba(223,255,0,.10); border-left: 5px solid #dfff00; padding: 12px; border-radius: 12px; margin-bottom: 12px; color: #eaffc4; }}
            .risk-pill {{ display: inline-block; padding: 5px 12px; border-radius: 999px; background: #dfff00; color: #102000; font-weight: 900; margin-left: 8px; }}
            .footer {{ color: #c5d6c5; font-size: 13px; margin-top: 18px; }}
            @media (max-width: 850px) {{ .grid {{ grid-template-columns: 1fr; }} .detail-row {{ grid-template-columns: 1fr; gap: 4px; }} }}
        </style>
    </head>
    <body>
        <div class='wrap'>
            <div class='top'>
                <h1>FRIS Grid Information Panel <span class='risk-pill'>{html.escape(priority)}</span></h1>
                <div class='sub'>
                    <b>Grid:</b> {html.escape(grid_id)}<br>
                    This is the full information layer for the selected grid. The map popup is intentionally kept small for field use.
                </div>
                <a class='btn' href='/'>Back to Dashboard</a>
                <a class='btn' href='/watchlist'>Open Watch-List</a>
                {nav_button}
            </div>
            <div class='grid'>
                {_detail_section('Operational Decision', overview_rows, 'Use this section first for patrol decision-making.')}
                {_detail_section('Vegetation & Moisture', vegetation_rows, 'NDVI/NDMI are satellite support values, not field proof by themselves.')}
                {_detail_section('Soil & Terrain Context', soil_rows, 'Soil context supports moisture interpretation and patrol planning.')}
                {_detail_section('Ecological Anomaly / Chronic Memory', watch_rows, 'This section keeps anomaly and chronic risk grids visible without the old long Ecology Inference table.')}
                {_detail_section('Fire & Weather Support', fire_weather_rows, 'Fire-risk/check is separate from confirmed active fire.')}
                {_detail_section('Canopy / Structure Support', canopy_rows, 'GEDI and tree-count values are support estimates, not exact tree enumeration.')}
                {_detail_section('Carbon MRV Support', carbon_rows, 'Carbon values are preliminary MRV-support indicators, not certified carbon-credit issuance.')}
            </div>
            <div class='footer'>FRIS grid information is generated from the latest CSV. Full audit values remain available in CSV download.</div>
        </div>
    </body>
    </html>
    """
...
def grid_details(grid_id):
    """Full information panel for a single FRIS grid.

    The map popup links here. This route reads the same lightweight CSV cache as
    the dashboard and fails gracefully if the grid is unavailable.
    """
    df = read_csv_light()
    if df is None or df.empty:
        return Response("FRIS CSV data not found. Run sks1a.py and make sure fris_latest.csv is available.", status=404, mimetype="text/plain")
    if "grid_id" not in df.columns:
        return Response("grid_id column not found in FRIS CSV.", status=404, mimetype="text/plain")

    target = str(grid_id).strip().upper()
    match = df[df["grid_id"].astype(str).str.strip().str.upper() == target]
    if match.empty:
        del df
        gc.collect()
        return Response(f"Grid {html.escape(grid_id)} not found in latest FRIS CSV.", status=404, mimetype="text/plain")

    row = match.iloc[0]
    page = build_grid_information_page(row)
    del df
    gc.collect()
