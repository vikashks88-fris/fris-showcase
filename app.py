...
def dashboard():
    ...
    if not os.path.exists(CSV_FILE):
        ...
        <p>Please keep files like this:</p>
        <pre>
fris_showcase/
├── app.py
├── requirements.txt
└── data/
    ...
    if len(priority_df) > 0 and available_priority_cols:
        priority_html = priority_df[available_priority_cols].head(50).to_html(
            classes="data-table",
            index=False,
            escape=False
        )
    else:
        priority_html = "<p>No priority grids found from current filter.</p>"
            ...
            <form class="filters" method="GET">
                ...
                <select name="health">
                    ...
                    {% for value in health_values %}
                        <option value="{{ value }}" {% if request.args.get('health') == value %}selected{% endif %}>
                            {{ value }}
                        </option>
                    {% endfor %}
                </select>
                ...
                <select name="moisture">
                    ...
                    {% for value in moisture_values %}
                        <option value="{{ value }}" {% if request.args.get('moisture') == value %}selected{% endif %}>
                            {{ value }}
                        </option>
                    {% endfor %}
                </select>
                ...
                <select name="risk">
                    ...
                    {% for value in risk_values %}
                        <option value="{{ value }}" {% if request.args.get('risk') == value %}selected{% endif %}>
                            {{ value }}
                        </option>
                    {% endfor %}
                </select>
