...
MAX_TABLE_ROWS = 20
MAX_WATCH_ROWS = 40
MAX_MAP_FEATURES = 350

IMPORTANT_COLUMNS = [
    "grid_id", "lat", "lon", "latitude", "longitude", "center_lat", "center_lon", "centroid_lat", "centroid_lon",
    ...
    "fire_count", "fire_detected", "active_fire", "fire_status", "patrol_action", "google_maps_link",
    "hansen_loss_pct", "mining_pressure_class",
    ...
    "ecosystem_carbon_total_ton", "estimated_ecosystem_carbon_ton",
    "carbon_change_ton", "carbon_change_co2e_ton", "carbon_change_from_365d",
    "temperature_c", "temperature", "temp_c", "rainfall_current_mm", "rainfall", "rain_mm", "rainfall_24h_mm",
    "wind_speed_kmph", "wind_kmph", "wind_speed", "wind_gust_kmph", "weather_fire_spread_class", "weather_provider"
]
...
def read_csv_light():
    ...
    try:
        ...
        if usecols:
            return pd.read_csv(CSV_FILE, usecols=usecols, low_memory=False)
...
def classify_ecological_watch(row):
    ...
    priority = safe_text(row.get("final_priority", row.get("risk_class", "")), "").upper()
    ...
    if fire_count > 0 or "FIRE" in priority or "CRITICAL" in priority:
        return {"category": "Fire Verification Alert", "level": "HIGH", "reason": "Active/recent fire or critical-priority signal is present.", "action": "Immediate or same-day field verification."}
    if forest_pct >= 85 and ndvi < 0.20:
        return {"category": "Ecological Anomaly Alert", "level": "HIGH", "reason": "Forest extent is high but NDVI is abnormally low.", "action": "Priority ecological field verification before any conclusion."}
    if hansen_loss >= 15 and forest_pct >= 85 and ndvi >= 0.40:
        return {"category": "Historical Disturbance Watch", "level": "MEDIUM", "reason": "Current forest appears stable, but historical forest-loss evidence is elevated.", "action": "Periodic ecological monitoring and disturbance verification."}
    if forest_pct >= 85 and ndvi < 0.40:
        return {"category": "Vegetation Stress Watch", "level": "MEDIUM", "reason": "Forest-dominant grid shows stressed vegetation signal.", "action": "Monitor in next runs and verify if stress continues."}
    ...
    if ndmi < -0.10 and forest_pct >= 85:
        return {"category": "Moisture Stress Watch", "level": "MEDIUM", "reason": "Forest-dominant grid shows weak moisture signal.", "action": "Monitor moisture trend and rainfall context."}
...
def build_priority_table(df, limit=MAX_TABLE_ROWS):
    ...
    for _, row in work.iterrows():
        ...
        <td>{format_number(row.get('ndvi'), 3)}</td><td>{format_number(row.get('ndmi'), 3)}</td><td>{int(safe_float(row.get('fire_count'), 0) or 0)}</td>
        <td>{format_number(row.get('final_risk_score'), 1)}</td><td>{html.escape(make_why_go(row))}</td><td>{html.escape(make_action(row))}</td>
        <td><a href='{html.escape(maps_link)}' target='_blank'>Open</a></td></tr>"""
    return f"<div class='table-wrap'><table><thead><tr><th>Grid</th><th>Priority</th><th>NDVI</th><th>NDMI</th><th>Fire</th><th>Risk</th><th>Why go?</th><th>Action</th><th>Map</th></tr></thead><tbody>{rows}</tbody></table></div>"
...
def ecological_summary_text(df):
    ...
    fire_rows = int(pd.to_numeric(df["fire_count"], errors="coerce").fillna(0).gt(0).sum()) if "fire_count" in df.columns else 0
...
def make_csv_grid_geojson(df):
    ...
    work = sorted_priority_df(df, MAX_MAP_FEATURES)
    features = []
    for _, row in work.iterrows():
        ...
        if lat is None or lon is None:
            continue
        d = 0.0045
        props = {c: (None if pd.isna(row.get(c)) else row.get(c)) for c in work.columns if not c.startswith("_")}
        watch = classify_ecological_watch(row)
        eco = make_ecology_inference(row)
        props.update({
    ...
    return {"type": "FeatureCollection", "features": features, "source": "CSV light priority-grid builder", "feature_limit": MAX_MAP_FEATURES}
...
def dashboard():
    ...
    active_fire = int(pd.to_numeric(df["fire_count"], errors="coerce").fillna(0).gt(0).sum()) if df is not None and "fire_count" in df.columns else 0
    watchlist_count = count_watchlist(df)
    avg_ndvi = avg_col(df, "ndvi")
    avg_ndmi = avg_col(df, "ndmi")
    forest_area_text = format_number(sum_first_available_col(df, ["effective_forest_area_ha", "forest_area_ha", "area_ha"]), 1, " ha")
    total_carbon_text = format_carbon(sum_first_available_col(df, ["ecosystem_carbon_total_ton", "estimated_ecosystem_carbon_ton"]))
    carbon_change_text = format_carbon(sum_first_available_col(df, ["carbon_change_ton", "carbon_change_co2e_ton", "carbon_change_from_365d"]))
...
<div class='main'><div class='topbar'><div class='time-box'><b>Current Time</b><span>{format_ist(ist_now())}</span></div><div class='time-box'><b>Last Data Update</b><span>{get_file_update_time(CSV_FILE)}</span><br><small>{get_file_age_minutes(CSV_FILE)}</small></div><div class='time-box'><b>Next Expected Run</b><span>{next_expected_run()}</span></div><div><a class='btn' href='/'>Refresh</a><a class='btn' href='/download/csv'>Download CSV</a><a class='btn' href='/download/map'>Download Map</a><a class='btn' href='/watchlist'>Watch-List</a><a class='btn' href='/debug'>Debug</a></div></div>
<div class='content' id='map-section'><div class='map-card'><h2>FRIS Operational Map</h2><div class='watch-note'>To save Render memory, this map shows the top {MAX_MAP_FEATURES} operational grids only. Full data remains downloadable.</div><div id='fris-map'></div></div><div class='right'><div class='card'><h3>📊 Operational Summary</h3><div class='row'><span>Total Grids</span><span class='value'>{total_grids}</span></div><div class='row'><span>High Risk</span><span class='value'>{high_risk}</span></div><div class='row'><span>Critical Risk</span><span class='value'>{critical_risk}</span></div><div class='row'><span>Active Fire</span><span class='value'>{active_fire}</span></div><div class='row'><span>Watch-List</span><span class='value'>{watchlist_count}</span></div></div><div class='card'><h3>💧 Forest Condition</h3><div class='row'><span>Average NDVI</span><span class='value'>{avg_ndvi}</span></div><div class='row'><span>Average NDMI</span><span class='value'>{avg_ndmi}</span></div></div><div class='card'><h3>🌳 Ecology Inference</h3><div class='watch-note'>{html.escape(summary)}</div></div><div class='card'><h3>🌦️ Weather</h3><div class='row'><span>Source</span><span class='value'>{html.escape(weather['source'])}</span></div><div class='row'><span>Temperature</span><span class='value'>{weather['temperature']}</span></div><div class='row'><span>Rainfall 24h</span><span class='value'>{weather['rainfall_24h']}</span></div><div class='row'><span>Wind</span><span class='value'>{weather['wind']}</span></div><div class='row'><span>Fire Spread</span><span class='value'>{html.escape(weather['spread'])}</span></div></div><div class='card'><h3>🌿 Carbon MRV</h3><div class='row'><span>Total Carbon</span><span class='value'>{total_carbon_text}</span></div><div class='row'><span>Carbon Change</span><span class='value'>{carbon_change_text}</span></div><div class='row'><span>Claim Status</span><span class='value'>Not Certified</span></div></div></div></div>
...
<div class='table-card' id='watchlist-section'><h2>🟣 Ecological Watch-List</h2><div class='watch-note'>Separate verification layer for fire, moisture stress, vegetation stress, historical disturbance, or influence-zone signals.</div>{watchlist_table}</div><div class='footer'>This app reads selected CSV columns only and limits web-map features to prevent Render memory crash.</div></div></div>
<script>
...
function popupHtml(p){{var link=p.google_maps_link||'#';return `<div style='font-family:Arial;min-width:260px'><b>${{p.grid_id||'Grid'}}</b><br><br><b>Priority:</b> ${{p.final_priority||p.risk_class||'N/A'}}<br><b>NDVI:</b> ${{num(p.ndvi)}}<br><b>NDMI:</b> ${{num(p.ndmi)}}<br><b>Fire:</b> ${{p.fire_count||0}}<br><b>Action:</b> ${{p.patrol_action||p.ecology_recommendation||'Routine patrol'}}<hr><b>Watch:</b> ${{p.ecological_watch_category||'Stable Forest Zone'}}<br><b>Why:</b> ${{p.ecological_watch_reason||'No major watch condition'}}<br><b>Inference:</b> ${{p.ecology_inference||'N/A'}}<br><br><small>Verification support only, not legal proof.</small><br><br><a href='${{link}}' target='_blank'>Open navigation</a></div>`}}
fetch('/geojson').then(r=>r.json()).then(data=>{{if(!data.features||data.features.length===0){{alert('No grid features found. Check CSV latitude/longitude columns.');return}}var layer=L.geoJSON(data,{{style:f=>{{var c=gridColor(f.properties||{{}});return{{color:c,weight:2,fillColor:c,fillOpacity:.55}}}},onEachFeature:(f,l)=>l.bindPopup(popupHtml(f.properties||{{}}))}}).addTo(map);try{{map.fitBounds(layer.getBounds(),{{padding:[20,20]}})}}catch(e){{}}L.control.layers({{'Satellite':satellite,'CartoDB Light':carto}},{{'FRIS Priority Grid Layer':layer}},{{collapsed:false}}).addTo(map)}}).catch(e=>alert('GeoJSON could not load. Open /debug.'));
</script></body></html>"""
    del df
    gc.collect()
...
def health():
    df = read_csv_light()
    lat_col, lon_col = find_lat_lon_columns(df)
    result = {"status": "running", "server_time_ist": format_ist(ist_now()), "csv_found": os.path.exists(CSV_FILE), "geojson_found": os.path.exists(GEOJSON_FILE), "map_found": os.path.exists(MAP_FILE), "csv_last_update": get_file_update_time(CSV_FILE), "csv_age": get_file_age_minutes(CSV_FILE), "next_expected_run": next_expected_run(), "total_grids": len(df) if df is not None else 0, "ecological_watchlist_count": count_watchlist(df), "csv_lat_column_found": lat_col, "csv_lon_column_found": lon_col}
    del df
    gc.collect()
