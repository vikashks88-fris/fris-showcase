...
# Dynamic time + true live weather + corrected map serving
# ============================================================
...
def ist_file_time(path):
    try:
        return datetime.fromtimestamp(path.stat().st_mtime, tz=IST).strftime(
            "%d %b %Y %I:%M %p IST"
        )
    except Exception:
...
def build_summary(df, meta, live_weather):
    ...
    return {
        ...
        "map_found": meta.get("map_found", False),
...
def map_view():
    if not LATEST_MAP.exists():
        return f"""
        <html>
        <body style="font-family:Arial;background:#06140f;color:white;padding:30px;">
            <h2>Map file not found</h2>
            <p>Expected map file:</p>
            <code>{LATEST_MAP}</code>
            <p>Correct location:</p>
            <code>data/fris_latest_map.html</code>
        </body>
        </html>
        """, 404
...
<aside class="sidebar">
    ...
    <nav class="nav">
        ...
        <a href="/map" target="_blank">🗺️ Open Map</a>
    ...
    <div class="side-info">
        ...
        <b>Map Status</b><br>
        {% if map_available %}
            <span style="color:#86ef45;">Map file found</span>
        {% else %}
            <span style="color:#ffd166;">Map file missing</span>
        {% endif %}
    </div>
</aside>
...
<main class="main">
    ...
    <div class="grid">
        <section class="map-card">
            <div class="map-title">
                LIVE RISK MAP<br>
                <span style="font-weight:normal;color:#ddd;">1 km Forest Grid</span><br>
                <a href="/map" target="_blank" style="color:#86ef45;font-size:12px;">Open full map</a>
            </div>

            {% if map_available %}
                <iframe id="riskMap" src="/map?ts={{ summary.last_updated }}"></iframe>
            {% else %}
                <div class="fake-map">
                    <div>
                        <h2>Map file not found</h2>
    ...
    <div class="footer">
        ...
        <span>FRIS CSV + Live Open-Meteo Weather + Corrected Map Route</span>
    </div>
...
function refreshMapFrame() {
    const frame = document.getElementById("riskMap");
    if (frame) {
...
// Refresh embedded map every 5 minutes
setInterval(refreshMapFrame, 300000);
</script>
